...
2. On the top exposed source row only, overlay the grass cap strip `(0..3, 0)` as a three-slice strip: column `0` is the left end, columns `1..2` are the repeating middle, and column `3` is the right end.
3. Do not repeat the whole cap block per gameplay cell; that repeats end caps through the middle of a continuous platform.
4. Do not paint the second cap/edge row as another grass band.
5. Do not overlay sparse grass-clump/decorative rows through the body of the ground.
6. Keep buried/stacked floor cells as pure opaque dirt backing.

This keeps the floor continuous while using the proper top grass strip from the tileset instead of sparse clumps, repeated cap ends, or a duplicate lower grass band.

## Continuous generated fill tiles

| Generated use | Atlas tile(s) | Reason |
| --- | --- | --- |
| Exposed lush grass cap overlay | `(0..3, 0)` | Single-row three-slice grass surface: left end, repeating middle, right end. |
...
- If the whole floor cell is exposed on top, source row `0` overlays the grass cap `(0..3, 0)`.
